...
export default function Detail() {
  ...
  const recommendationMovies = useSelector(
    (state) => state.movies.recommendationMovies
  );
  const nowPlayingHoveredMovieId = useSelector(
    (state) => state.movies.nowPlayingMoviesHoveredId
  );
  // const rating = useSelector((state) => state.movies.ratingResponse)
  ...
  const handleSubmitRating = () => {
    if (userRating) {
      dispatch(postAddRating(id, userRating));
    }
  };

  const handleSetRating = (rating) => {
    setUserRating(rating);
    // Post the rating immediately when a star is clicked
    if (id) {
      dispatch(postAddRating(id, rating));
    }
  };
  ...
  return (
    <div className="">
      ...
      <div className="bg-no-repeat bg-white-auto justify-around w-full">
        <div className="text-white relative" key={movieDetail?.id}>
          <img
      ...
      >
        <div className="flex flex-1 pl-3">
          ...
          <div className="flex flex-col ml-4 pl-8  w-[800px] pt-10">
            ...
            <p className="text-lg mb-3  pb-3 w-[600px]">
              {movieDetail?.overview}
            </p>
            <p className="border-b-2 pb-3 w-[770px]"></p>

            {/* rating */}
            <p className="text-3xl font-bold mb-3 pt-3  pb-3">Rating</p>
            <p className="text-lg  w-[600px]"></p>
            <div className="flex">
            ...
            <div>
              ...
              <div style={{ overflowX: "auto" }}>
                <div style={{ display: "flex", flexDirection: "row" }}>
                  {movieVideos &&
                    movieVideos.results.map((video, i) => (
                      <div
                        key={video.id}
                        style={{
                          display: "flex",
        ...
        <div className="section px-12">
          <div className="flex flex-row justify-between">
            <div className="flex justify-center items-center">
              <h1 className="pt-2 sm:pt-9 text-[12px] sm:text-[30px] font-bold text-black">
                Rekomendasi
              </h1>
            </div>
          ...
          <div className="section-scroll-container overflow-x-auto overflow-y-hidden whitespace-nowrap py-5 px-1">
            <div className="scrolling-wrapper-flexbox flex">
              <div className="scrolling-wrapper-flexbox flex">
                {recommendationMovies?.map((movie2, i) => (
                  <div
                    key={i}
                    onClick={() => {
                      // navigate("detail");
                      ...
                      dispatch(setMovieId(movie2.id));
                      navigate("/detail");
                      dispatch(getDetailMovie(movie2.id));
                      dispatch(getMovieReviews(movie2.id));
                      dispatch(getRecommendation());
                    }}
